...
export default function AccountSetupScreen() {
  ...
  const finish = async () => {
    ...
    if (!accountName.trim())
      return Alert.alert("Add an account name or skip setup");
    if (!Number.isFinite(parsed) || parsed < 0)
      return Alert.alert("Enter a valid opening balance");
    setSaving(true);
    try {
      await create.mutateAsync({
        name: accountName.trim(),
        type: "cash",
        openingBalanceMinor: Math.round(parsed * 100),
      });
      router.replace("/dashboard");
    } finally {
      setSaving(false);
    ...
    <Screen>
      <ScrollView contentContainerStyle={{ paddingTop: 30, paddingBottom: 30 }}>
        ...
        <Text style={styles.subtitle}>
          A wallet, bank account, or savings pot. You can always do this later.
        </Text>
        <Field
          label="Account name"
          value={accountName}
        ...
        />
        <Button
          title={saving ? "Creating..." : "Finish setup"}
          onPress={finish}
        />
        <Button
