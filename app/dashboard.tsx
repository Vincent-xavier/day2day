...
export default function DashboardScreen() {
  ...
  const loading =
    accountsLoading || transactionsLoading || lendingLoading || tasksLoading;
  const hasError =
    accountsError || transactionsError || lendingError || tasksError;
  const retryAll = () => {
    refetchAccounts();
    refetchTransactions();
    refetchLending();
  ...
  const activeLending = lending.filter(
    (item) => item.status === "active",
  ).length;
  const openTasks = tasks.filter((task) => !task.completed).length;
  const today = new Date().toISOString().slice(0, 10);
  const reminders = lending.filter(
    (item) =>
  ...
  const nextAction = useMemo(() => {
    if (accounts.length === 0) {
      return {
        ...
        detail:
          "This is the only step left before Day2Day can show your real balance.",
        cta: "Add account",
        route: "/accounts/new" as Href,
      };
    }
    if (reminders.length > 0) {
      ...
      return {
        eyebrow:
          reminders.length > 1
            ? `${reminders.length} BALANCES NEED YOU`
            : "NEEDS YOU TODAY",
        headline: `Follow up with ${item.personName}`,
        detail: `${item.name} · ${item.dueAt === today ? "due today" : `was due ${item.dueAt}`}`,
        cta: "Open ledger",
        route: "/lending" as Href,
      };
    }
    if (transactions.length === 0) {
      return {
        eyebrow: "GET STARTED",
        headline: "Log your first transaction",
        detail:
          "A single entry is enough to start seeing your real balance move.",
        cta: "Add transaction",
        route: "/transactions/new" as Href,
      };
    }
    if (openTasks > 0) {
      return {
        eyebrow: "NEXT STEP",
        headline: `Finish ${openTasks} open task${openTasks === 1 ? "" : "s"}`,
        detail: "Small, specific actions keep today moving.",
        cta: "Open tasks",
        route: "/tasks" as Href,
      };
    }
    return {
      ...
      detail: "Add a transaction whenever money moves.",
      cta: "Add transaction",
      route: "/transactions/new" as Href,
    };
  ...
  const navigate = (
    ...
    if (destination === "settings")
      return router.push("/settings" as unknown as Href);
    ...
    <Screen>
      <ScrollView contentContainerStyle={{ paddingBottom: 18 }}>
        ...
        {hasError ? (
          ...
          <LoadingState label="Getting your space ready..." />
        ) : (
          <>
            <Card
              style={{
            ...
            <View style={styles.row}>
              <Card style={styles.stat}>
                <Text style={styles.muted}>Open ledgers</Text>
                <Text style={styles.statValue}>{activeLending}</Text>
              </Card>
              <Card style={styles.stat}>
                <Text style={styles.muted}>Open tasks</Text>
                <Text style={styles.statValue}>{openTasks}</Text>
              </Card>
            </View>
          </>
        )}
