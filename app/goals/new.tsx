...
export default function NewGoalScreen() {
  ...
  const save = async () => {
    ...
    if (!Number.isFinite(parsed) || parsed <= 0)
      return Alert.alert("Enter a valid target");
    await create.mutateAsync({
      name: name.trim(),
      targetMinor: Math.round(parsed * 100),
    });
    router.back();
  };
  return (
    <Screen>
      <ScrollView contentContainerStyle={{ paddingTop: 8, paddingBottom: 30 }}>
        ...
        <Text style={styles.subtitle}>
          Set a target you can make progress toward.
        </Text>
        <Field
          label="Goal name"
          value={name}
        ...
        />
        <Button
          title={create.isPending ? "Saving..." : "Save goal"}
          onPress={save}
