...
export default function SetupScreen() {
  ...
  const [profile, setProfile] = useState<"personal" | "business" | "both">(
    "personal",
  );
  const finish = async () => {
    if (!name.trim()) return;
    await SecureStore.setItemAsync(
      "day2day_profile",
      JSON.stringify({ name: name.trim(), profile }),
    );
    ...
    <Screen>
      <ScrollView contentContainerStyle={{ paddingTop: 30, paddingBottom: 30 }}>
        ...
        <Text style={styles.subtitle}>
          Just the basics for now. You can change these anytime.
        </Text>
        <Field
          ...
          autoFocus
        />
        <Text style={styles.label}>How will you use Day2Day?</Text>
        ...
        <Button title="Continue" onPress={finish} disabled={!name.trim()} />
        <Button
          title="Skip for now"
          variant="ghost"
